    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if not boxes:
                ...
                TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap', status='no_boxes').inc()
            ...
            TEMPERATURE_BOXES_COUNT.labels(status='processed').set(len(boxes_to_process))
            print(f"Boxes to process: {[box_id[:8] + '...' for box_id in boxes_to_process]}")
            ...
            TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap', status='success').inc()
    ...
    try:
        async with session.get(url, params=params, timeout=15) as response:
            if response.status == 200:
                ...
                TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_boxes', status='success').inc()
                return data
            else:
                print(f"Error: HTTP {response.status}")
                TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_boxes', status=f'http_{response.status}').inc()
                return []
    except asyncio.TimeoutError:
        print("Timeout fetching boxes list")
        TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_boxes', status='timeout').inc()
        return []
    except aiohttp.ClientError as e:
        print(f"Network error: {e}")
        TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_boxes', status='network_error').inc()
        return []
    except Exception as e:
        print(f"Unexpected error: {e}")
        TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_boxes', status='unexpected_error').inc()
        return []
    finally:
        duration = time.time() - start_time
        TEMPERATURE_API_DURATION.labels(api_source='opensensemap_boxes').observe(duration)
    ...
    for box in boxes:
        ...
        try:
            ...
            if time_diff <= timedelta(hours=hours):
                ...
                print(f"↩ Fallback box {box['_id'][:8]}... updated {time_diff.total_seconds()/3600:.1f} hours ago")
...
async def get_boxes_temp(box_ids, session: aiohttp.ClientSession) -> list:
    ...
    async def fetch_single_box_temp(box_id: str) -> float | None:
        start_time = time.time()
        try:
            async with session.get(f"https://api.opensensemap.org/boxes/{box_id}", timeout=10) as response:
                if response.status == 200:
                    data = await response.json()
                    temp_values = []
                    for sensor in data.get("sensors", []):
                        ...
                        if (any(keyword in sensor_title for keyword in ["temperature", "temp", "t°", "lufttemperatur"]) or 
                            "temperature" in sensor_type) and last_measurement:
                            ...
                            if temp_value and temp_value.strip():
                                try:
                                    ...
                                    if -20 <= temp_float <= 40:
                                        ...
                                        print(f"🌡 Box {box_id[:8]}... temp: {temp_float}°C")
                                except ValueError:
                                    continue

                    if temp_values:
                        TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_sensor', status='success').inc()
                        return sum(temp_values) / len(temp_values)
                    else:
                        print(f"❌ No valid temperature data for box {box_id[:8]}...")
                        TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_sensor', status='no_data').inc()
                        return None
                else:
                    print(f"🔴 HTTP {response.status} for box {box_id[:8]}...")
                    TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_sensor', status=f'http_{response.status}').inc()
                    return None
        except asyncio.TimeoutError:
            print(f"⏰ Timeout fetching box {box_id[:8]}...")
            TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_sensor', status='timeout').inc()
            return None
        except Exception as e:
            print(f"Unexpected error with box {box_id[:8]}: {e}")
            TEMPERATURE_API_REQUESTS.labels(api_source='opensensemap_sensor', status='error').inc()
            return None
        finally:
            duration = time.time() - start_time
            TEMPERATURE_API_DURATION.labels(api_source='opensensemap_sensor').observe(duration)
        ...
        async with session.get(url, params=params, timeout=10) as response:
            if response.status == 200:
                ...
                if temp is not None:
                    ...
                    TEMPERATURE_API_REQUESTS.labels(api_source='openmeteo', status='success').inc()
                    TEMPERATURE_DATA_QUALITY.set(0)
                    return float(temp)
            else:
                TEMPERATURE_API_REQUESTS.labels(api_source='openmeteo', status=f'http_{response.status}').inc()
    except Exception as e:
